...
export default function CreateArticle({ id }) {
    ...
    const FormValidation = Yup.object({
        ...
        file: Yup.mixed()
            ...
            .test({
                message: 'فقط فرمت png و jpg مجاز است.',
                test: (file, context) => {
                    if (id && !file) {
                        return true
                    }
                    const isValid = ['png', 'jpg'].includes(file?.name?.split('.').pop());
    ...
    return (
        <div>
            <h3 className='page-title'>افزودن مقاله</h3>
            <form className='mb-4' onSubmit={formik.handleSubmit}>
                ...
                <div className="text-strt">
                    <input type="submit" disabled={isLoading} value="افزودن مقاله"
                        className="form-submit !py-2 block w-full" />
                </div>
            </form>
        </div>
    )
